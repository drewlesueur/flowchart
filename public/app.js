...
function layoutDecision(model, statement, cx, topY) {
  ...
  if (thenLayout.entry) {
    ...
    model.edges.push(createEdge([leftPort, { x: leftPort.x - 26, y: leftPort.y }, { x: leftPort.x - 26, y: mergeY }, { x: cx, y: mergeY }], "yes"));
  }

  if (elseLayout.entry) {
    connectOrthogonal(model, rightPort, elseLayout.entry, elseLayout.entry.x, statement.elseBranch.length ? "no" : "");
  } else {
    model.edges.push(createEdge([rightPort, { x: rightPort.x + 26, y: rightPort.y }, { x: rightPort.x + 26, y: mergeY }, { x: cx, y: mergeY }], statement.elseBranch.length ? "no" : ""));
  }
...
function renderNode(node) {
  const stroke = CHART_COLORS.stroke;
  const fill =
    node.type === "start"
      ? CHART_COLORS.start
      : node.type === "end"
        ...
        : node.type === "decision"
          ...
          : node.type === "group"
            ? CHART_COLORS.group
            : node.type === "terminal"
              ? CHART_COLORS.terminal
              : CHART_COLORS.process;
  ...
  if (node.type === "decision") {
    ...
    return `<g class="node decision"><polygon points="${points}" fill="${fill}" stroke="${stroke}" stroke-width="2.2"/>${renderText(node)}</g>`;
  }

  return `<g class="node ${node.type}">
    <rect x="${node.x}" y="${node.y}" width="${node.width}" height="${node.height}" rx="${STYLE.cornerRadius}" fill="${fill}" stroke="${stroke}" stroke-width="2.2"/>
    ${renderText(node)}
  </g>`;
}
...
function renderSvg(model) {
  ...
  const remappedNodes = model.nodes.map((node) => ({
    ...node,
    x: node.x + offsetX,
  }));

  const remappedEdges = model.edges.map((edge) => ({
    ...edge,
    points: edge.points.map((point) => ({ x: point.x + offsetX, y: point.y })),
  }));

  return `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="Flow chart">
    <defs>
    ...
    ${remappedEdges.map(renderEdge).join("")}
    ${remappedNodes.map(renderNode).join("")}
  </svg>`;
}

function renderEmptyState(message) {
  svgMount.innerHTML = `<div class="empty-state">${escapeHtml(message)}</div>`;
}
...
function updateChart() {
  ...
  if (!source) {
    renderEmptyState("Paste some code on the left to generate an SVG flow chart.");
